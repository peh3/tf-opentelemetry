...
# Node 1: OpenTelemetry Collector
...
resource "aws_instance" "app" {
  ...
  user_data = <<-EOF
    ...
    def create_order():
        ...
        return {"status": "Order processed", "order_id": random.randint(1000, 9999)}
...
# Node 6: Jenkins Controller (With explicit JVM heap caps for t3.micro)
resource "aws_instance" "jenkins" {
  ...
  user_data = <<-EOF
    ...
    # Limit JVM heap to 512MB so Jenkins can run cleanly within 1GB RAM
